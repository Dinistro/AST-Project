...
if
gcc -fsanitize=undefined -Wstrict-prototypes -Wfatal-errors -pedantic -Wall -Wsystem-headers -O3 -I${CSMITH_HOME}/runtime ./initial.c -o ./tmp.out > $TMP1 2>&1  &&\
! grep 'conversions than data arguments' "$TMP0" &&\
! grep 'incompatible redeclaration' "$TMP0" &&\
! grep 'ordered comparison between pointer' "$TMP0" &&\
! grep 'eliding middle term' "$TMP0" &&\
! grep 'end of non-void function' "$TMP0" &&\
! grep 'invalid in C99' "$TMP0" &&\
! grep 'specifies type' "$TMP0" &&\
! grep 'should return a value' "$TMP0" &&\
! grep 'uninitialized' "$TMP0" &&\
! grep 'incompatible pointer to' "$TMP0" &&\
! grep 'incompatible integer to' "$TMP0" &&\
#! grep 'comparison of distinct pointer types' "$TMP0" &&\
! grep 'type specifier missing' "$TMP0" &&\
...
(ulimit -t 5; ./tmp.out | grep "checksum = 0")
if ! test "$?" = "0"; then
    exit 1
fi 
