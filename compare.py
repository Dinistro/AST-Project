...
subprocess.run('mkdir compf', cwd='./', text=True, timeout=60, shell=True)
...
for cmd in toRun:
    if(args.printCmd):
        print(cmd)
    subprocess.run(cmd, cwd=directory, text=True, timeout=60, shell=True)
...
if args.nodelete:
    ...
    subprocess.run('rmdir compf', cwd='./', text=True, timeout=60, shell=True)
